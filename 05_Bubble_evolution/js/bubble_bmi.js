 function bubble_evo() // generate heatmap 
	{ 	
		var file_use = "data/bmi_evolution.csv"; 
	...
	// Add axis, title and x-title to the graph
		...
		if (bool_left_graph) {
			...
			v_key = 1 
			axis_orient = "right"
			graph_width =  graph_width+ spaceRightAxes
			axis_x = graph_width
	...
	function add_bar_text_line(graph, data,bool_left_graph) {
	   ...
	   nodes.append("line") // add tick for each group
		 ...
		 nodes.append("circle")
			.attr("class","circle1")
			.attr("r", 20)
			.style("stroke", color_by[v_key])   // set the line colour
			.style("stroke-width", 2)
	  ...
	  nodes.append("line")
		...
		nodes.append("text")
			...
			.attr("transform", function(d, i) {return "translate(0," + (yScale(d.rate1)) + ")";}) 
			.attr("dy", "0.25em")
			.attr("fill", color_by[v_key]);    // set the line colour
		...
		d3.csv(file_use,
			...
			function(data) {
				...
				var data_src = d3.nest()
            		.key( function(d){ return d.var_graph ;  })
            		.entries( data )  ; 
	...
	function update_data_circle(graph, data,bool_left_graph,data_src) {
		...
		graph_select.selectAll(".text1")
			...
			.tween("text", function(d,i) {
				var to = d.rate1 ;
				if (data_src != null) {
					var from = data_src[0].values[d.var_graph-1].values[i].rate1 ; 
				}
				else {
					var from = to
				} 				 
		...
		graph_select.selectAll(".text2")
			...
			.tween("text", function(d,i) {
				if (bool_new) {
				 var to = d.rate2 ;
				  if (data_src != null) {
					var from = data_src[0].values[d.var_graph-1].values[i].rate2 ; 
				  }	else {
					var from = to
				  }
				} else {
		...
		graph_select.selectAll(".text_percent")
			...
			.tween("text", function(d,i) {
				...
				if (data_src != null) {
					var from = (data_src[0].values[d.var_graph-1].values[i].rate2 - data_src[0].values[d.var_graph-1].values[i].rate1)/
								(data_src[0].values[d.var_graph-1].values[i].rate1);
				} else {
					var from = to
				} 
	...
	//generate tick on the axis 
		...
		var log_diff = Math.pow(10,Math.floor(Math.log10(value_max - value_min))); // order of magnitude of min (power of 10)
		var unit_floor_diff = Math.floor((value_max-value_min)/log_diff) // left digit of min 
		
		if (!log_scale) {
			var tick_space = 0;
			if (unit_floor_diff < 2) {
				tick_space = 0.2*log_diff;	
			}
			else {
				if (unit_floor_diff < 5) {
					tick_space = 0.5*log_diff;
					}
				else {
				tick_space = log_diff;
				}
